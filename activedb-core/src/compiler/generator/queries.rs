...
impl Query {
                ...
                if struct_def.is_primitive {
                    ...
                    for (field_idx, field) in struct_def.fields.iter().enumerate() {
                        ...
                        let field_value = match &field_info.source {
                            crate::compiler::generator::return_values::ReturnFieldSource::NestedTraversal {
                            ...
                            _ if matches!(
                                ...
                                crate::compiler::generator::return_values::ReturnFieldType::Nested(_)
                            ) => {
                                // Nested struct or object - need to construct recursively
                                let nested_fields = match &field_info.field_type {
                                    crate::compiler::generator::return_values::ReturnFieldType::Nested(fields) => fields,
                                    _ => {
                                        debug_assert!(false, "Type invariant: expected Nested field type after matches! guard");
                                        static EMPTY: Vec<crate::compiler::generator::return_values::ReturnFieldInfo> = Vec::new();
                                        &EMPTY
                                    }
                                };
                    ...
                    // Use AQL closure param name if available, otherwise fall back to singular form
                    ...
                    for (field_idx, field) in struct_def.fields.iter().enumerate() {
                        let field_value = if field.is_nested_traversal {
                            ...
                            if let crate::compiler::generator::return_values::ReturnFieldSource::NestedTraversal {
                                ...
                                let is_singular_traversal_value = matches!(
                                    &field_info.field_type,
                                    crate::compiler::generator::return_values::ReturnFieldType::Simple(ty) if ty == &RustFieldType::TraversalValue
                                );

                                let is_vec_traversal_value = matches!(
                                    &field_info.field_type,
                                    crate::compiler::generator::return_values::ReturnFieldType::Simple(RustFieldType::Vec(inner)) if inner.as_ref() == &RustFieldType::TraversalValue
                                );

                                // Check if this is a Vec<Value> (from graph navigation returning mapped values)
                                let is_vec_value = matches!(
                                    &field_info.field_type,
                                    crate::compiler::generator::return_values::ReturnFieldType::Simple(RustFieldType::Vec(inner)) if inner.as_ref() == &RustFieldType::Value
                                );

                                // Check if this is an Option<Value> (from ::FIRST)
                                let is_option_value = matches!(
                                    &field_info.field_type,
                                    crate::compiler::generator::return_values::ReturnFieldType::Simple(RustFieldType::OptionValue)
                                );
                            ...
                            // or anonymous traversals (e.g., creatorID: _::In<Created>::ID)
                            } else if let crate::compiler::generator::return_values::ReturnFieldSource::NestedTraversal {
                                closure_source_var: Some(source_var),
                                ...
                                let is_value_variable_ref = matches!(&field_info.field_type,
                                    crate::compiler::generator::return_values::ReturnFieldType::Simple(RustFieldType::Value))
                                ...
                                for nested_field in nested_fields {
                                    ...
                                    let nested_val = if let crate::compiler::generator::return_values::ReturnFieldSource::NestedTraversal {
                                        ...
                                        let inner_fields_str = if let crate::compiler::generator::return_values::ReturnFieldType::Nested(inner_fields) = &nested_field.field_type {
                                        ...
                                        let (access_var, accessed_field_name) = if let crate::compiler::generator::return_values::ReturnFieldSource::NestedTraversal {
                                            closure_source_var: Some(source_var),
                                            accessed_field_name: accessed_field,
                                            ..
                                        } = &nested_field.source {
                                        ...
                                        } else if let crate::compiler::generator::return_values::ReturnFieldSource::ImplicitField { property_name: Some(prop) } = &nested_field.source {
                                            // Case 3: Implicit field with remapped name (e.g., postID: id)
                                            (closure_param, prop.as_str())
                                        } else if let crate::compiler::generator::return_values::ReturnFieldSource::SchemaField { property_name: Some(prop) } = &nested_field.source {
                                ...
                                let is_variable_ref = trav_code.trim().is_empty()
                                    && (traversal_type.is_none() || matches!(traversal_type, Some(crate::compiler::generator::traversal_steps::TraversalType::Empty | crate::compiler::generator::traversal_steps::TraversalType::Ref)));
                            ...
                            if let crate::compiler::generator::return_values::ReturnFieldSource::ComputedExpression { expression } = &field_info.source {
                                use crate::compiler::generator::computed_expr::generate_computed_expression;
                                generate_computed_expression(expression, singular_var)
                            } else {
                                let property_name = match &field_info.source {
                                    crate::compiler::generator::return_values::ReturnFieldSource::ImplicitField { property_name } => {
                                        property_name.as_ref().map(|s| s.as_str()).unwrap_or(&field.name)
                                    },
                                    crate::compiler::generator::return_values::ReturnFieldSource::SchemaField { property_name } => {
                                        property_name.as_ref().map(|s| s.as_str()).unwrap_or(&field.name)
                                    },
                                    _ => &field.name,
                                };
    ...
    fn print_mcp(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if self.use_struct_returns && !self.return_structs.is_empty() {
            ...
            writeln!(
                f,
                "Ok(activedb_core::protocol::format::Format::Json.create_response(&response))"
            )?;
        } else if !self.return_values.is_empty() {
            // Legacy json! macro approach
            write!(f, "let response = json!({{")?;
            ...
            writeln!(
                f,
                "Ok(activedb_core::protocol::format::Format::Json.create_response(&()))"
            )?;
        }
        if !self.hoisted_embedding_calls.is_empty() {
            writeln!(f, r#"}}))).await.expect("Cont Channel should be alive")"#)?;
