...
pub const GITHUB_ISSUE_URL: &str = "https://github.com/ActiveDB/activedb-engine/issues/new";
...
error: could not compile `activedb-container` due to 1 previous error
"#;
   ...
   --> activedb-container/src/queries.rs:192:43
    |
192 | .insert_v::<fn(&HVector, &RoTxn) -> bool>(&data.vec, "File8Vec", Some(...
    |  ---------------------------------------- ^^^^^^^^^ expected `&[f64]`, found `&Vec<f32>`

error[E0308]: mismatched types
   --> activedb-container/src/queries.rs:194:43

error: could not compile `activedb-container` due to 2 previous errors
"#;
    ...
    fn test_extract_first_error_none() {
        let cargo_output = "error: could not compile `activedb-container`";
        let first_error = extract_first_error(cargo_output);
