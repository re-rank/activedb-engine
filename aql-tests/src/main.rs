...
impl GitHubConfig {
    fn from_env() -> Result<Self> {
        ...
        let owner = env::var("GITHUB_OWNER").unwrap_or_else(|_| "ActiveDB".to_string());
        let repo = env::var("GITHUB_REPO").unwrap_or_else(|_| "activedb-engine".to_string());
...
async fn main() -> Result<()> {
    let matches = ClapCommand::new("queries-test")
        .about("Process activedb test directories")
        .arg(
            Arg::new("test_name")
    ...
    let project_root = match current_dir.parent() {
        Some(parent) if parent.join("activedb-cli").exists() => parent.to_path_buf(),
        Some(_) if current_dir.join("activedb-cli").exists() => current_dir.to_path_buf(),
    ...
    // build rust cli from ./activedb-engine/activedb-cli with sh build.sh dev
    println!("DEBUG: Building rust cli from ./activedb-engine/activedb-cli with sh build.sh dev");
    let build_script_path = project_root.join("activedb-cli/build.sh");
    println!("DEBUG: Build script path: {}", build_script_path.display());
    println!("DEBUG: Build script exists: {}", build_script_path.exists());

    let activedb_cli_dir = project_root.join("activedb-cli");
    println!("DEBUG: ActiveDB CLI dir: {}", activedb_cli_dir.display());
    println!("DEBUG: ActiveDB CLI dir exists: {}", activedb_cli_dir.exists());

    // Check if activedb is already available
    let activedb_check = Command::new("activedb").arg("--version").output();

    match activedb_check {
        Ok(output) => {
            if output.status.success() {
                println!(
                    "DEBUG: ActiveDB already available: {}",
                    String::from_utf8_lossy(&output.stdout)
                );
            } else {
                println!("DEBUG: ActiveDB not available or failed version check");
            }
        }
        Err(e) => {
            println!("DEBUG: ActiveDB command not found: {e}");
        }
    }
    ...
    let output = Command::new("sh")
        .arg("build.sh")
        .arg("dev")
        .current_dir(&activedb_cli_dir) // Change to activedb-cli directory first
        .output()
    ...
    if !output.status.success() {
        bail!(
            "[FAILED] BUILD FAILED: activedb-cli build.sh failed\nStderr: {}\nStdout: {}",
            String::from_utf8_lossy(&output.stderr),
            String::from_utf8_lossy(&output.stdout)
        );
    } else {
        ...
        // Check if activedb is available after build
        let activedb_check_after = Command::new("activedb").arg("--version").output();

        match activedb_check_after {
            Ok(output) => {
                if output.status.success() {
                    println!(
                        "DEBUG: ActiveDB available after build: {}",
                        String::from_utf8_lossy(&output.stdout)
                    );
                } else {
                    println!("DEBUG: ActiveDB still not available after build");
                    println!(
                        "DEBUG: ActiveDB version check stderr: {}",
                        String::from_utf8_lossy(&output.stderr)
                    );
                }
            }
            Err(e) => {
                println!("DEBUG: ActiveDB command still not found after build: {e}");
            }
        }
    }
...
async fn process_test_directory(
    ...
    let temp_dir = env::temp_dir().join(format!("activedb_temp_{test_name}"));
    if temp_dir.exists() {
    ...
    // Copy the test files (queries.hx, schema.hx, activedb.toml, etc.) to temp directory
    copy_dir_recursive(&folder_path, &temp_dir).await?;

    // Copy the entire activedb-engine project structure for cargo check
    // But skip .activedb directory to avoid conflicts
    let activedb_core_dir = temp_dir.join("activedb-engine");
    fs::create_dir_all(&activedb_core_dir).await?;
    ...
    for crate_name in crates_to_copy {
        ...
        let dst = activedb_core_dir.join(crate_name);
        if src.exists() {
    ...
    let cargo_toml_dst = activedb_core_dir.join("Cargo.toml");
    ...
    let cargo_lock_dst = activedb_core_dir.join("Cargo.lock");
    if cargo_lock_src.exists() {
        fs::copy(&cargo_lock_src, &cargo_lock_dst).await?;
    }

    // Run activedb compile command
    let compile_output_path = temp_dir.join("activedb-engine/activedb-container/src");
    fs::create_dir_all(&compile_output_path)
        .await
        .context("Failed to create compile output directory")?;

    let output = Command::new("activedb")
        .arg("compile")
        .arg("--path")
        .arg(&temp_dir)
        .arg("--output")
        ...
        .output()
        .context("Failed to execute activedb compile command")?;

    println!(
        "DEBUG: ActiveDB compile output: {}",
        String::from_utf8_lossy(&output.stdout)
    );
    println!(
        "DEBUG: ActiveDB compile stderr: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    if !output.status.success() {
        ...
        // For activedb compilation, we'll show the raw output since it's not cargo format
        ...
        if let Some(config) = github_config {
            println!("DEBUG: ActiveDB compilation failed in parallel mode, creating GitHub issue...");
    ...
    // Run cargo check on the activedb container path
    let activedb_container_path = temp_dir.join("activedb-engine/activedb-container");
    if activedb_container_path.exists() {
        let output = Command::new("cargo")
            .arg("check")
            .current_dir(&activedb_container_path)
            .output()
...
const IGNORE_DIRS: [&str; 3] = ["target", ".git", ".activedb"];
